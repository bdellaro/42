...
int	ft_check_error_map(char *data_map, t_set *content, t_solong *solong)
{
	...
	while (data_map[i] != 0)
	{
		...
		if (solong->data_map[i] != content->player && solong->data_map[i] != content->exit &&
			solong->data_map[i] != content->collect && solong->data_map[i] != content->wall &&
			solong->data_map[i] != content->floor && solong->data_map[i] != content->sl)
		{
			ft_printf("Error\nInvalid map\n");
			free(data_map);
			exit(EXIT_FAILURE);
